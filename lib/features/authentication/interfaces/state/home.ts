...
export const dispatchLoginSanityUseCaseAtom = atom(
  null,
  async (_get, set, payload: LoginSanityPayload) => {
    set(baseLoading, true);
    try {
      const useCase = AppIOCContainer.get('LoginSanityUseCase');
      const result = await useCase.execute(payload.jwtToken);
